...
public class ConfigProperties {
    ...
    private static final String USERNAME_PROPERTIES_KEY = "account.username";

    private static final String PASSWORD_PROPERTIES_KEY = "account.password";
    ...
    static {
        ...
        try ( InputStream in = ConfigProperties.class.getResourceAsStream( "/" + PROPERTIES_FILE ) ) {
            ...
            USER_CREDENTIALS = loadUserAccount( properties );
        } catch ( final IOException e ) {
            LOG.error( "Configuration could not be loaded!", e );
            throw new UncheckedIOException( e );
        }
    }

    private static Optional<UserAccount> loadUserAccount( final Properties properties ) {
        return Optional.ofNullable( properties.getProperty( USERNAME_PROPERTIES_KEY ) ).map( user -> {
